...
public class CoffeeService {
  ...
  //커피 메뉴를 관리하는 싱글톤 객체(프로그램에서 한개만 존재)
  ...
  private void end() {
    ...
    for(Map.Entry<String,Integer> order : customer.getCoffeeOder().entrySet()){
  ...
  private void totalOrder(Customer customer2) {
    ...
    for(Map.Entry<String,Integer> order : customer.getCoffeeOder().entrySet()){
      ...
      int coffeeUnitPrice = coffee.menu.get(coffeeName); //커피 객단가
      // Americano 3000
      // .get(coffeeName) : 커피 이름으로 값을 조회
  ...
  private void payment(int totalMoney) {
    ...
    int payResult = customer.getMoney() - totalMoney; //고객 잔고 - 결제 총금액
    try {
      System.out.println("\n결제중입니다.");
      t.sleep(2000); //2초후 실행
    ...
    if (payResult < 0) {
      ...
      customer.setMoney(payResult); //결제 후 잔액을 고객잔고에 넣어놓기
  ...
  private void order() {
    System.out.println("\n 취소를 원하시면 0번을 눌러주세요.");
      ...
      customer.setCoffeeOder(orderList); //커피이름, 수량을 주문리스트에 담아서 저장
      //추가주문
      addOrder();
      break end;
      } catch (Exception e) {
